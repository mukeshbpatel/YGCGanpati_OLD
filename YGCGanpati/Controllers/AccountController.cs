...
namespace YGCGanpati.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, Name = model.Name, FlatNo = model.FlatNo };
        ...
        public ActionResult ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = UserManager.ResetPassword(user.Id, code, "abcd@1234");
                if (result.Succeeded)
                {
                    if (!string.IsNullOrEmpty(user.Email))
                    {
                        EmailNotification ygcemail = new EmailNotification();
                        ygcemail.SendForgetPasswordEmail(user,"abcd@1234");
                        ygcemail = null;
                    }
